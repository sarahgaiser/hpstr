...
//   cuts = {"unc_vtx_ele_track_clust_dt": [-7.8, 7.8],
//                     "unc_vtx_pos_track_clust_dt": [-6.6, 6.6],
//                     "unc_vtx_ele_track_chi2ndf": [0., 20],
//                     "unc_vtx_pos_track_chi2ndf": [0., 20],
//                     "unc_vtx_ele_track_p": [0.4, 2.9],
//                     "unc_vtx_pos_track_p": [0.4, 4.],
//                     "unc_vtx_ele_track_nhits": [8., 100.],
//                     "unc_vtx_pos_track_nhits": [8., 100.],
//                     "unc_vtx_chi2": [0., 30.],
//                     "unc_vtx_psum": [0., 4.],
//                     "unc_vtx_pos_clust_E": [0.2, 4.]}

  vertex_cf_.add("pos_cluster_E", 100, 0.0, 4.0);
  vertex_cf_.add("ele_track_pos_cluster_dt", 100, -10.0, 10.0);
  vertex_cf_.add("pos_track_pos_cluster_dt", 100, -10.0, 10.0);
  vertex_cf_.add("ele_track_chi2ndf", 100, 0.0, 40.0);
  vertex_cf_.add("pos_track_chi2ndf", 100, 0.0, 40.0);
  vertex_cf_.add("ele_track_pmin", 100, 0.0, 4.0);
  vertex_cf_.add("ele_track_pmax", 100, 0.0, 4.0);
  vertex_cf_.add("pos_track_pmin", 100, 0.0, 4.0);
  vertex_cf_.add("ele_n_hits", 14, 0, 14);
  vertex_cf_.add("pos_n_hits", 14, 0, 14);
  vertex_cf_.add("vertex_chi2", 100, 0.0, 50.0);
  vertex_cf_.add("vertex_psum", 100, 0.0, 6.0);
  vertex_cf_.init();
...
//   }
  ...
  for (Vertex* vtx : vtxs) {
    ...
    double ele_track_cluster_tdiff{
          ele.getTrack().getTrackTime()-pos.getCluster().getTime()
    };
    double pos_track_cluster_tdiff{
          pos.getTrack().getTrackTime()-pos.getCluster().getTime()
    };
    TVector3 ele_mom(
...
//                     "unc_vtx_pos_clust_E": [0.2, 4.]}
    ...
    vertex_cf_.apply("pos_cluster_E", ele.getCluster().getEnergy() >= 0.2);
    vertex_cf_.apply("ele_track_pos_cluster_dt", abs(ele_track_cluster_tdiff) <= 7.8);
    vertex_cf_.apply("pos_track_pos_cluster_dt", abs(pos_track_cluster_tdiff) <= 6.6);
    vertex_cf_.apply("ele_track_pmin", ele_mom.Mag() >= 0.4);
    vertex_cf_.apply("ele_track_pmax", ele_mom.Mag() <= 2.9);
    vertex_cf_.apply("pos_track_pmin", pos_mom.Mag() >= 0.4);
    ...
    vertex_cf_.apply("ele_n_hits", ele_nhits >= 10);
    vertex_cf_.apply("pos_n_hits", pos_nhits >= 10);
    vertex_cf_.apply("vertex_chi2", vtx->getChi2() <= 20.0);
    vertex_cf_.apply("vertex_psum", (ele_mom+pos_mom).Mag() <= 4.0);

    vertex_cf_.fill_nm1("pos_cluster_E", pos.getCluster().getEnergy());
    vertex_cf_.fill_nm1("ele_track_pos_cluster_dt", ele_track_cluster_tdiff);
    vertex_cf_.fill_nm1("pos_track_pos_cluster_dt", pos_track_cluster_tdiff);
    vertex_cf_.fill_nm1("ele_track_pmin", ele_mom.Mag());
    vertex_cf_.fill_nm1("ele_track_pmax", ele_mom.Mag());
    vertex_cf_.fill_nm1("pos_track_pmin", pos_mom.Mag());
    ...
    vertex_cf_.fill_nm1("ele_n_hits", ele_nhits);
    vertex_cf_.fill_nm1("pos_n_hits", pos_nhits);
    vertex_cf_.fill_nm1("vertex_chi2", vtx->getChi2());
    vertex_cf_.fill_nm1("vertex_psum", (ele_mom+pos_mom).Mag());
  ...
  if (bus_.has(mcColl_) and isSignal_) {
    ...
    event_cf_.apply("at_least_one_true_ap", nAp > 0);
    event_cf_.apply("no_extra_true_ap", nAp < 2);
    if (not event_cf_.keep()) {
      return true;
    }
    if (not ap) {
